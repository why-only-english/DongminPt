...
### 2. GitHub Actions (비밀번호 해시 주입 + 자동 배포)

`.github/workflows/deploy.yml`이 main 브랜치 push 시 자동으로 실행된다.
...
### 3. 비밀번호 해시 설정 (GitHub Secret)

1. 사용할 비밀번호의 SHA-256 해시 생성 — 브라우저 콘솔에서 실행:
   ```javascript
   crypto.subtle.digest('SHA-256', new TextEncoder().encode('여기에비밀번호')).then(b => console.log([...new Uint8Array(b)].map(x=>x.toString(16).padStart(2,'0')).join('')))
   ```
2. `Settings → Secrets and variables → Actions → New repository secret`
3. Name: `PASSWORD_HASH`, Value: 위에서 생성한 해시값 (64자리 hex)
...
> **주의**: PAT를 코드에 직접 작성하지 말 것. 설정 모달에서 입력하면 브라우저 localStorage에만 저장된다.
